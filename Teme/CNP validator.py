...
# oare este formatul de cnp corect, cifre sa fie exact 13 and so on
    if len(cnp) != 13:
        ...
        nnn = cnp[9:12]
        c = cnp[-1]
# validatus bucati din validator cu cifrele din tabel
        sex_ok = '1' <= sex <= '9'
        an_ok = '00' <= an <= '99'
        luna_ok = '01' <= luna <= '12'
        zi_ok = '01' <= zi <= '31'
        jj_ok = '01' <= jj <= '52'
        nnn_ok = '001' <= nnn <= '999'
# incepe cifra de control de la 0 si definesc formual de control
        c_control = 0
...
# some very complicated siens and math
        if c_control == 10:
            c_control = 1
# well if it is equal to then it must be equal to
